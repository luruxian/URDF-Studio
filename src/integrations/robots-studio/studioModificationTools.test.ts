...
test('createParseToolCalls parses propose_requirements_revision', () => {
  const parseToolCalls = createParseToolCalls('en');
  const parsed = parseToolCalls([
    {
      function: {
  ...
  assert.ok(parsed);
  assert.equal(parsed.toolName, 'propose_requirements_revision');
  assert.equal(parsed.summary, 'Extend arm by 5cm');
});
...
test('onExecute for propose_requirements_revision runs PATCH → regenerate → poll → grant → import', async () => {
  ...
  assert.deepEqual(JSON.parse(String(spy.calls[1].init?.body)), {
    base_revision: 3,
    change_summary: 'arm +5cm',
    append_markdown: '## v4\n',
  });
