...
LLM 调用 `edit_robot_appearance` 时，`prompt` 参数必须为 **JSON 字符串**，按以下结构生成。JSON 结构化比自然语言段落更可控，减少 jimeng 对提示词不同维度的理解偏差。系统指令中应明确要求 LLM 遵循此规范。

#### 5.4.1 JSON Schema

```json
{
  "subject": "<主体描述：材质、颜色、形状、结构细节>",
  "preserve": "<保留约束：必须保留的元素>",
  "size": "512x512",
  "style": "<风格/质量：渲染风格与质量标准>",
  "negative": "<负面约束：不要改变的内容>"
}
```
...
| 字段 | 内容 | 示例值 |
|------|------|--------|
| `subject` | 将用户需求翻译为精确的视觉描述，使用具体颜色名和色值 | `"机器人机身改为亮橙色(#FF8C00)，保留金属拉丝质感"` |
| `preserve` | 必须保留的元素 | `"保持原有摄像机视角、机器人结构、关节位置和背景完全不变"` |
| `size` | 输出尺寸，固定值 | `"512x512"` |
| `style` | 渲染风格与质量标准 | `"工业设计渲染风格，影棚灯光，高清晰度，photorealistic"` |
| `negative` | 不要改变的内容 | `"不要改变机器人结构、不要变形、不要添加文字或logo"` |
...
```
当用户请求修改机器人外观时，你必须生成结构化的 JSON 提示词，而非直接使用用户原话。
JSON 包含以下五个字段：

- subject：将用户需求翻译为精确的视觉描述（材质、颜色、形状、结构），
...
你必须输出合法的 JSON 字符串（单行，不含换行和注释），作为 prompt 参数的值。

示例——用户说"把机身改成橙色"，prompt 参数的值应为：
...
确认 UI 中展示的 `summary` 为 LLM 用简短中文概括的用户意图（如「将机身改为橙色，保留原视角」），而非完整的 JSON prompt。JSON prompt 通过 API 直接传给 jimeng，不展示给用户。
