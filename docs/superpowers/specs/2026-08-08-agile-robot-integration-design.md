...
| `regenerate_robot_3d` | 直接用现有图片重新生成 3D | 无 | hunyuan/submit → 轮询 → reload mesh |
...
LLM (MiniMax function calling)
  │ 返回 tool_calls: [{ name: "edit_robot_appearance", arguments: { prompt: "把机身改成橙色，保留原视角" } }]
  ▼
